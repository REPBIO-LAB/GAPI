...
if __name__ == '__main__':
	...
	## 1. Define parser ##
	### Mandatory arguments
	parser = argparse.ArgumentParser(description='Call viral integrations from third generation sequencing data. Two running modes: 1) SINGLE: individual sample; 2) PAIRED: tumour and matched normal sample')
	...
	parser.add_argument('--readFilters', default=None, dest='readFilters', type=str, help='Comma separated list of read filters to apply (SMS)')
	...
	parser.add_argument('--minCLIPPINGlen', default=500, dest='minCLIPPINGlen', type=int, help='Minimum clipped sequence length for each read. Default [ILLUMINA, SURESELECT]: 20; [NANOPORE, PACBIO]: 500')
	...
	# load Internal
	import VIGA_LR.call_VIGALR as call_VIGALR
	import log
	import bamtools
	...
	# Hard-coded for virus
	confDict['transductionSearch'] = False
	confDict['germlineMEI'] = None
	
	## 2. Execute structural variation caller
	###########################################

	### Create caller
	caller = call_VIGALR.SV_caller_long(mode, bam, normalBam, reference, refDir, confDict, outDir)
