...
def write_INS(INS_metaclusters, outFileName, outDir):
    ...
    ## 2. Write header 
    row = "#ref \t beg \t end \t filters \t mutOrigin \t insType \t family \t subfamily \t cytobandId \t gnName \t biotype \t region \t gene \t families \t subfamilies \t nbTotal \t nbTumour \t nbNormal \t nbINS \t nbDEL \t nbCLIPPING \t length \t cv \t percResolved \t qHits \t tHits \t insertSeq \n"
    outFile.write(row)
