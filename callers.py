...
class SV_caller_long(SV_caller):
    ...
    def call(self):
        ...
        for tipo, metaPass in allMetaclusters.items():
            for metPass in metaPass:
                print ('allMetaclustersTipo ' + str(tipo) + ' '+ str(metPass.beg))
        ...
        if 'INS' in allMetaclusters:
            ...
            clusters.INS_type_metaclusters(allMetaclusters['INS'], self.reference, self.refLengths, self.refDir, self.confDict['transductionSearch'], 1, self.confDict['viralDb'], outDir)
            # NOTE 2020: New 2020
        ...
        if 'BND' in allMetaclusters:
            ...
            unix.mkdir(outDir)   
            allMetaclusters['BND'] = clusters.search4bridges_metaclusters_parallel(allMetaclusters['BND'], 10000, 80, self.confDict['minSupportingReads'], 25, self.refLengths, self.refDir, self.confDict['viralDb'], self.confDict['processes'], outDir)
            # NOTE 2020: New 2020, but I will continue with commited one
            ...
            for metaBND1 in allMetaclusters['BND']:
                print ('metaBND1 ' + str(metaBND1.beg))

            # NOTE 2020: AQUI DESAPARECEN TODOS LOS REALES!!!
            ### Search for BND junctions
            # NOTE 2020: La dejo como estaba
            allJunctions = clusters.search4junctions_metaclusters(allMetaclusters['BND'], self.refLengths, self.confDict['processes'], self.confDict['minSupportingReads'], 25, self.reference, self.refDir, self.confDict['viralDb'], outDir)
    ...
    def make_clusters(self):
        ...
        '''
        for metaP3 in metaclustersPass.values():
            for mP3 in metaP3:
                print ('metaPmake_clusters ' + str(mP3.beg))

        for metaF3 in metaclustersFailed.values():
            for mF3 in metaF3:
                print ('metaFmake_clusters ' + str(mF3.beg))
        '''
    ...
    def make_clusters_bin(self, ref, beg, end):
        ...
        '''
        for metaP in metaclustersSVType.values():
            for mP in metaP:
                print ('metaPBfCons ' + str(mP.beg))

        for metaF in metaclustersSVTypeFailed.values():
            for mF in metaF:
                print ('metaFBfCons ' + str(mF.beg))
        '''
        ...
        '''
        for metaP2 in metaclustersSVType.values():
            for mP2 in metaP2:
                print ('metaPAfCons ' + str(mP2.beg))

        for metaF2 in metaclustersSVTypeFailed.values():
            for mF2 in metaF2:
                print ('metaFAfCons ' + str(mF2.beg))
        '''
